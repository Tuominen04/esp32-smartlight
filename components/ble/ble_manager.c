...
static void ble_manager_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
  ...
  case ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT:
    ...
    if (ret != ESP_OK) {
      ESP_LOGE(GAP_TAG, "Failed to start advertising after adv data set: %s", esp_err_to_name(ret));
    }
    break;
  case ESP_GAP_BLE_ADV_DATA_RAW_SET_COMPLETE_EVT:
    ret = esp_ble_gap_start_advertising(&adv_params);
    if (ret != ESP_OK) {
      ESP_LOGE(GAP_TAG, "Failed to start advertising after raw adv data set: %s", esp_err_to_name(ret));
    }
    break;
  case ESP_GAP_BLE_SCAN_RSP_DATA_SET_COMPLETE_EVT:
    ret = esp_ble_gap_start_advertising(&adv_params);
    if (ret != ESP_OK) {
      ESP_LOGE(GAP_TAG, "Failed to start advertising after scan response set: %s", esp_err_to_name(ret));
    }
    break;
...
case ESP_GATTS_WRITE_EVT:
  ...
  // Handle CCCD descriptor writes (for enabling notifications)
    ...
    esp_err_t ret = esp_ble_gatts_send_response(gatts_if, param->write.conn_id, param->write.trans_id, ESP_GATT_OK, NULL);
    if (ret != ESP_OK) {
      ESP_LOGE(GATTS_TAG, "Send default response failed, error code = %x", ret);
    }
  }
  break;
...
void ble_manager_handle_device_info_confirmation(void *pv_parameters)
{
  ...
  while (1) {
    if (waiting_confirmation) {
      ESP_LOGI(BLE_TAG, "Handling confirmation");
      ...
      if (!confirmation_successful) {
        ...
        if (err == ESP_OK) {
          ...
          esp_err_t adv_ret = esp_ble_gap_start_advertising(&adv_params); // Restart BLE advertising to try again
          if (adv_ret != ESP_OK) {
            ESP_LOGE(GAP_TAG, "Failed to restart advertising: %s", esp_err_to_name(adv_ret));
          }
        } else {
          ESP_LOGE(BLE_TAG, "Failed to delete WiFi credentials: %s", esp_err_to_name(err));
        }
      } else {
...
esp_err_t ble_manager_init(void)
{
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(BLE_TAG, "Failed to release classic BT memory: %s", esp_err_to_name(ret));
    return ret;
  }
